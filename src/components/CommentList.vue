<template>
  <div class="comments_wrap space-y-4 sm:space-y-5 mt-10">
    <template v-if="comments.length">
      <div
        v-for="(comment, index) in comments"
        :key="index"
        class="p-5 md:p-5 bg-gray-100 dark:bg-Fizico-bg text-zinc-800 dark:text-slate-100 rounded-xl"
      >
        <div
          class="flex items-center justify-between pb-4 mb-4 border-b border-b-neutral-200/60 dark:border-white/10"
        >
          <div class="flex items-center gap-x-3.5">
            <div
              class="border-Fizico-color2 w-15 h-15 border rounded-full relative"
            >
              <div
            ...
            <div class="flex flex-col gap-1">
              <span class="inline-block max-w-40 truncate">{{ username }}</span>
              <span class="font-DanaBlack text-sm opacity-70">{{
                comment.date
              }}</span>
            </div>
          </div>
        ...
        <p class="font-DanaBlack font-normal text-sm sm:text-base break-words">
          {{ comment.text }}
        </p>
      </div>
    </template>
    ...
    <div
      v-else
      class="flex flex-col items-center justify-center gap-y-3  text-gray-500 text-center Font-DanaLight">
      <svg
        class="block"
        width="112"
        height="100"
      ...
      <p>هنوز نظری ثبت نشده! تو اولین نفر باش</p>
    </div>
  </div>
</template>
